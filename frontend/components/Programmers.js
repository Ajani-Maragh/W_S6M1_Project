...
export default function Programmers() {
  ...
  const [currentPioneer, setCurrentPioneer] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return pioneers.find(pi => pi.id === currentPioneer).name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: currentPioneer ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          pioneers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setCurrentPioneer(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
